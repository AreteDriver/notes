...
**Project:** ANIMUS, GameSpace, vdc-core (new)

**Decision Class:** ARCH

**Decision:** Add Protocol interface contracts alongside existing ABCs; extract pure domain logic to vdc-core shared package

**Chosen Option:**
- Create `typing.Protocol` interfaces in each project for structural contracts
- New `vdc-core` Python package contains shift calculation logic extracted from vdc-portfolio
- Protocols marked with `@runtime_checkable` for `isinstance()` conformance testing
- Protocols coexist with ABCs; no replacement of existing inheritance hierarchies

**Rejected Options:**
- Replace ABCs entirely with Protocols (loses nominal type checking, breaks existing inheritance)
- No formalization of interfaces (leaves contracts implicit)
- Monolithic vdc-portfolio logic (violates separation of concerns for shared domain)

**Why:**
- Protocol pattern formalizes structural contracts that enable mock/stub flexibility
- `isinstance()` with `@runtime_checkable` provides conformance testing without inheritance coupling
- Extracting shift calculations to vdc-core creates reusable domain logic for multiple apps
- Protocols + ABCs is complementary: ABC for inheritance guarantees, Protocol for structural verification

**Implementation Details:**
- Use `@runtime_checkable` on all Protocols for isinstance() support
- For conformance testing with properties: `instance = ClassName.__new__(ClassName)` then `isinstance(instance, Protocol)`
- vdc-core contains pure calculations (no IO, no dependencies beyond stdlib)
- Consumers import shift logic: `from vdc_core.shifts import calculate_shift_workload`

**Tradeoffs Accepted:**
- Extra abstraction layer (Protocols alongside ABCs)
- Must mark Protocols with `@runtime_checkable` for isinstance() to work
- Protocol structural checks are runtime, not compile-time (no mypy enforcement without plugin)

**Revisit If:**
- Protocols and ABCs create confusion in codebase (consolidate into single interface pattern)
- vdc-core logic becomes stateful (move back into vdc-portfolio)
